...
def get_test_response():
    ...
    session_attributes = {}
    card_title = "test"
    compliments = ["you","me","him"]
    print(compliments)
    compliment = 'nán zǐ qù mǎi chē ，xū yào shí wàn yuán ，kě nán zǐ zhī dài le xiàn jīn jiǔ wàn yuán ，jiù chà liǎng yuán qián ！tū rán\
        tā fā xiàn mén kǒu yǒu yī gè qǐ gài ，jiù guò qù duì qǐ gài shuō ：“qiú nǐ le ，gěi wǒ liǎng yuán qián ba ，wǒ yào mǎi chē ！\
        qǐ gài tīng hòu ，dà fāng dì ná chū sì yuán qián dì gěi nán zǐ ，shuō ：“bāng wǒ yě mǎi yī liàng .'
    #compliment = "liu shao jin shi zui shuai de,her jia shi zui chou de'
    speech_output = compliment
    reprompt_text = "I said," + compliment
    should_end_session = False
    return build_response(session_attributes, build_speechlet_response(
...
def get_count_response(intent):
    ...
    iiii=""
    try:
        i=int(intent['slots']['num']['value'])
    except:
        i=1

    try:
        ii=intent['slots']['word']['value']
    except:
        ii=""

    try:
        iii=intent['slots']['address']['value']
    except:
        iii=""
    try:
        iiii=intent['slots']['thing']['value']
    except:
        iiii=""
    speech_output =''
    reprompt_text = "Counting... "

    should_end_session = False

    for item in range(i):
        speech_output=speech_output+str(item)+' '+ii+iii+iiii+' '
        # TODO: write code...
    return build_response(session_attributes, build_speechlet_response(
        card_title, speech_output, reprompt_text, should_end_session))
...
def get_dontknow_response():
    """ If we wanted to initialize the session to have some attributes we could
    add those here
    """
    session_attributes = {}
    card_title = "I don't know"
    speech_output = "I don't know, can you say it again"
    # If the user either does not reply to the welcome message or says something
    # that is not understood, they will be prompted again with this text.
    reprompt_text = "I don't know "
    should_end_session = False
    return build_response(session_attributes, build_speechlet_response(
        card_title, speech_output, reprompt_text, should_end_session))
...
def on_intent(intent_request, session):
    ...
    if intent_name == "Compliment":
        ...
        return get_count_response(intent)
